...
try {
	...
	currentBuild.result = 'FAILURE'
  	...
  	stage ('Sending status'){
		sh "cd ${JENKINS_HOME}/jobs/123-multibranch/branches/${BRANCH_NAME}/builds/${BUILD_NUMBER}"
		~ sh "grep Aborted log | sed 's/\(^Aborted by \)\(.*\)=\s*\(.*\)/\1\3/'"
		echo "RESULT: ${currentBuild.result} - ${result} - ${rest}"
	}
    }
}
